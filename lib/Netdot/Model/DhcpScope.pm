...
=head2 as_text -  Print the config file as text (ISC DHCPD format)
...
sub as_text{
    ...
    my $text;
    # Open an in-memory file handle
    open(MH, ">", \$text) or $self->throw_fatal("Can't open memory file: $!");

    print MH "###############################################################\n";
    print MH "# Generated by Netdot (http://netdot.uoregon.edu)\n";
    print MH "###############################################################\n\n";

    $class->_print(\*MH, $self->id, $data);

    print MH $EOF_MARKER;

    $logger->debug(sprintf("DHCPD Scope %s generated in %s", 
			   $self->name, $class->sec2dhms(time-$start)));
...
sub print_to_file{
    ...
    $logger->info(sprintf("DHCPD Scope %s exported to %s", 
			  $self->name, $path));
}
