...
=head2 auto_allocate -  Auto-allocate Block
    ...
    For a given block, and given the desired length, this method will
  ...
  Arguments: 
    Hash with following keys:
    length    prefix length of the desired sub-block
    strategy, one of:
  ...
  Examples:
    my $sub_block = $ipblock->auto_allocate(length=>23, strategy=>'first')

=cut
sub auto_allocate {
    my ($self, %args) = @_;
    $self->isa_object_method('auto_allocate');
    my $class = ref($self);

    $args{strategy} ||= "first";  # this will be the default

    unless ( $args{strategy} =~ /^first|last$/ ){
	$self->throw_fatal("Unknown strategy: $args{strategy}");	
    }
    ...
    $logger->debug("Ipblock::auto_allocate: parent: %s/%s, len: %s, strategy: %s" ,
		   $self->address, $self->prefix, $args{length}, $args{strategy});

    if ($self->prefix >= $args{length}){
	$self->throw_user("New block's prefix length must be larger than parent's");
    }    

    my $wantaddress = 0;
    if ( ( $self->version == 4 && $args{length} == 32 ) ||
	 ( $self->version == 6 && $args{length} == 128 ) ){
	$wantaddress = 1;
    }
    if ( $wantaddress ){
	# Only under Subnet
	unless ( $self->status->name eq "Subnet" ){
	    $self->throw_user("Address allocations only permitted within 'Subnet' blocks");
	}
    }else{ # want a block
	# Only under Container
	unless ( $self->status->name eq "Container" ){
	    $self->throw_user("Block allocations only permitted within 'Container' blocks");
	}
    }
    # We need the NetAddr::IP object
    my $parent_nip = $self->_netaddr;
    ...
    # where m is $args{length}.  So, we need to try all combinations of bits
    ...
    my $increment = 2**($parent_nip->bits - $args{length});
    my $min_ip    = $parent_nip;
    my $total_ips = 2**($args{length} - $parent_nip->masklen);
    my $max_ip    = $min_ip + ($total_ips - 1)*$increment; # -1 since we start with the first address.
    my $start_ip  = $min_ip;
    # Get the children in *ascending* order as NetAddr::IP objects
    my @children = map { $_->_netaddr() } 
                         $self->children({ order_by => 'address' });
    if ($args{strategy} eq "last") {
	$start_ip   = $max_ip;
	$increment *= -1;
	# Descending order
	...
	if ( @children ) {
		...
		$logger->debug(sub { sprintf("Ipblock::auto_allocate: %s conflicts with %s, skipping %s blocks, 
                                              next child is %s",
					     $block, $children[0], $number_of_conflicted_blocks->($children[0]),
		...
		$logger->debug(sub { sprintf ("Ipblock::auto_allocate: %s does not conflict with %s",
					      $block, $children[0]) });
		$i += 1;
	    }
	}
    ...
    for ( $i = 0; $i < $total_ips; ) {
	$block = NetAddr::IP->new ( ($start_ip + $i*$increment)->addr, $args{length} );
	
	$process_conflicts_and_iterate->();

	$logger->debug(sub { sprintf("Ipblock::auto_allocate: trying %s/%s" ,
		       $block->addr, $block->masklen) });
	...
	if ( my $ipblock = $class->search( address => $block->addr, 
	    ...
	    if ( $newblock = $self->insert( \%state ) ){
		
		$logger->debug(sub { sprintf("Ipblock::auto_allocate: returned %s/%s" ,
					     $newblock->address, $newblock->prefix) });
