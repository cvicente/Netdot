...
sub _validate_arp {
	...
	unless ( $intid ) {
	    $logger->warn("Device::CiscoCat::_validate_arp: $host: Could not match $iname to any interface name");
	    next;
	}
	foreach my $ip ( keys %{$cache->{$key}} ){
		...
		if ( $version == 6 && !Netdot->config->get('IGNORE_IPV6_LINK_LOCAL') ){
			...
			$valid{$intid}{$ip} = $mac;
			$logger->debug(sub{"Device::CiscoCat::_validate_arp: $host: valid: $iname -> $ip -> $mac" });
			next;
		    }
		}
		foreach my $nsub ( @{$devsubnets{$intid}} ){
		    my $nip = NetAddr::IP->new($ip) or
			$self->throw_fatal(sprintf("Device::CiscoCat::_validate_arp: Cannot create NetAddr::IP ".
						   "object from %s", $ip));
		    if ( $nip->within($nsub) ){
			$valid{$intid}{$ip} = $mac;
			$logger->debug(sub{"Device::CiscoCat::_validate_arp: $host: valid: $iname -> $ip -> $mac" });
			...
			$logger->debug(sub{"Device::CiscoCat::_validate_arp: $host: $ip not within $nsub" });
		    }
		}
	    }else{
		$valid{$intid}{$ip} = $mac;
