...
foreach my $dt (keys %tree){
    ...
    unless (@products = Product->search(type => "$type")){
	warn "Couldn't find any products of type $dt\n)";
	next;
    }
    foreach my $product ( @products ){
	my $name = $product->name;
	if ( my @devs = Device->search(productname => $product) ){
    ...
    @devices = sort { $a->name->name cmp $b->name->name } @devices;
	...
	my @ports;
	unless ( @ports = Interface->search(device => $o) ){
	    next;
	}
	@ports = sort { $a->number <=> $b->number } @ports;
	foreach my $p (@ports){
